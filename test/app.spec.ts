...
describe('App', () => {
    ...
    it('should throw user not found error when trying to authentication'), async () => {
        const app = new App()
        expect(() => {app.authenticate('jos@email.com', '1234')}).toThrow(UserNotFoundError)
    }

    it('should correctly handle register user'), async () => {
        const app = new App()
        const user = new User('Jose', 'jose@mail.com', '1234')
        app.registerUser(user)
        expect(app.users.length).toEqual(1)
        expect(app.users[0].email).toEqual('jose@email.com')
        expect(app.users[0].name).toEqual('jose')
    }

    it('should correctly remove register user'), async () => {
        const app = new App()
        const user = new User('Jose', 'jose@mail.com', '1234')
        app.registerUser(user)
        app.removeUser('jose@email.com')
